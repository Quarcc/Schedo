...
for TASKs IF, and ONLY IF it is before the dueDate.
  ...
  try {
    const completion = await openai.chat.completions.create({
      messages: [{
        ...
        {
          role: "user",
          content: `I will now provide you an EVENT I need you to insert into the schedule, and the existing schedule. Please remember that the date and timings of EVENTs cannot be moved around.
        TASK to be added: ${taskToAdd}
        Exisiting EVENTs and TASKs: ${existingEventsTasks}
        BLOCK-OFF TIMES AND PREFERRED DAYS/TIMES: ${personalization}
  ...
  try {
    const completion = await openai.chat.completions.create({
      messages: [{
        ...
        Using the above information, your role is to OPTIMIZE the schedule such that all tasks are done before the deadline AND that the event is added into the schedule. The EVENT HAS to be scheduled on the specific date and time, so reschedule any tasks in that time slot if there are existing tasks. Remember your instructions and the restrictions put on EVENTs and BLOCKED-OFF TIMES. DO NOT REMOVE any EVENTs or TASKs - it is IMPERATIVE that all the tasks are preserved and the new one is added.
